...
This package comes with a sister package, MSGFplus, that provides the interface
...
This will open up MSGFgui in your standard browser. Unfortunately due to a 
timing issue in how shiny sets up the server, it is necessary to reload the page
once before everything works as expected. This will probably be fixed before the
next Bioconductor release so maybe it doesn't apply to you at the time of 
reading this. But to be sure, just do it anyway.

Once the GUI is running the R process is interupted. To regain control of R (and
shut down MSGFgui) press the 'esc' key.
